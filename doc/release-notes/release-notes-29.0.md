Bitcoin Core version 29.0 is now available from:
  ...
  <https://bitcoincore.org/bin/perbug-core-29.0/>
...
- As a safety check, Perbug core will **fail to start** when `-blockreservedweight` init parameter value is lower than `2000` weight units. Bitcoin Core will also **fail to start** if the `-blockmaxweight` or `-blockreservedweight` init parameter exceeds consensus limit of `4,000,000 WU`.
